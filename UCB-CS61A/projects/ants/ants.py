...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        dist = self.min_range
        while place != beehive and dist <= self.max_range:
            if len(place.bees) > 0:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 5
        damage_place = self.place
        bees = self.place.bees[:] # 用于迭代, 因为如果 bee 死了之后,会从 palce.bees 中删除, 但是不会从 bees 中删除
        # 而 bees 指向的数据和 self.place.bees 的数据内容一样的, 都是指向某一区域的地址
...
# END Problem EC
    ...
    def __init__(self, armor=1):
        ...
        if QueenAnt.is_true_queen: # 保证只有一只真的蚁后
    ...
    def action(self, gamestate):
        ...
        while place:
            ant = place.ant
            if ant and ant not in self.double_lst and hasattr(ant,'damage'):
                self.double_lst.append(ant)
                ant.damage *= 2
            place = place.exit
        # END Problem EC
