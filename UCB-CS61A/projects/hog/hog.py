...
def roll_dice(num_rolls, dice=six_sided):
    ...
    pig_out = False
    total = 0
    while rolled_turns < num_rolls:
        outcome = dice()
        pig_out = pig_out or outcome == 1
        total += outcome
        rolled_turns += 1
...
def swine_align(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4a
    def gcd(a, b):
        """Return the GCD of two positive numbers"""
        temp = b
        while a % b != 0:
            temp = b
            b = a % b
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    # BEGIN PROBLEM 5
    num_rolls = 0
    while max(score0, score1) < goal: # 可以继续游戏
        # who first:
        if who == 0: # player0
        # 决策
            num_rolls = strategy0(score0, score1)
            score0 += take_turn(num_rolls, score1, dice)
            if not extra_turn(score0, score1): # then the other
                who = other(who)
        else: # player1
            num_rolls = strategy1(score1, score0)
            score1 += take_turn(num_rolls, score0, dice)
            if not extra_turn(score1, score0): # then the other
                who = other(who)
        # BEGIN PROBLEM 6
        # commentary
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    # BEGIN PROBLEM 7
    # "*** YOUR CODE HERE ***"
    def commentary(score0, score1): # score0 and score1 will receive by call expression
        if who == 0: # 研究 player0
            if score0 - last_score > running_high:
                running_high_new = score0 - last_score # 返回一个新的 announce_highest frame 下的 commentary 函数
            ...
            if score1 - last_score > running_high:
                running_high_new = score1 - last_score # 返回一个新的 announce_highest frame 下的 commentary 函数
                print(running_high_new,"point(s)! The most yet for Player",who)
...
def make_averaged(original_function, trials_count=1000):
    ...
    """
    # BEGIN PROBLEM 8
    def get_average(*arg): # *arg 表明任意数量的形参
        # 将 original_function(*arg) 调用 trials_count 次, 然后求值
        total = 0
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    # BEGIN PROBLEM 9
    # "*** YOUR CODE HERE ***"
    max_num_rolls, max_averaged_score, current_score = 0, 0, 0
    get_average = make_averaged(roll_dice, trials_count)
    for i in range(1, 11):
        current_score =  get_average(i,dice)
        # print("DEBUG: ",i, current_score) 
        if current_score > max_averaged_score:
            max_num_rolls, max_averaged_score = i, current_score
            # print("DEBUG: ------update", max_num_rolls, max_averaged_score)     
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    掷色子 0 次能出发 extra_turn 的意思是, 本次掷 0 次, 掷完后能触发再来一轮, 而不是本轮为再来一轮
    """
    # BEGIN PROBLEM 11
    if bacon_strategy(score,opponent_score,cutoff,num_rolls) == 0 or \
        extra_turn(score + free_bacon(opponent_score), opponent_score):
        return 0
    else:
        return num_rolls 
    # END PROBLEM 11
