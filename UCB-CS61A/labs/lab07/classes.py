...
class Player:
    ...
    def draw(self):
        ...
        self.hand[-1:-1] = [self.deck.draw()] # 用切片来删减, 一定要注意右值是 list, 而不是 element
...
class TutorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        True
        """
        opponent.hand[0:3] = [] # discard the first 3 cards
        opponent.hand[-1:-1] = [opponent.deck.draw() for _ in range(3)]
...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        for card in opponent_cards:
            if card.attack == other_card.attack or card.defense == other_card.deffense:
