...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        Number(14)
        """
        # self.operator.eval(env) 对操作符 PrimitiveFunction 求含义, apply(...) 使用, 
        # PrimitiveFunction.apply(arg) 只接受 Number class list
        # 因此用 list comprehesion 将 Name, Literal 转换成 Number
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        parent_env.update({i[0]:i[1] for i in zip(self.parameters, arguments)})
